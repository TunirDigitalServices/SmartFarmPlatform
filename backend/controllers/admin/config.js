...
const getCountries = async (req, res) => {
  try {
    const countries = await new Countries()
      ...
      .then(async (result) => {
        if (result == null)
          return res.status(404).json({ type: "danger", message: "no_cities" });
        if (result)
          return res.status(201).json({ type: "success", Countries: result });
      });
  } catch (error) {
    ...
    const countries = await new Countries({ iso: iso })
      ...
      .then(async (result) => {
        if (result === null)
          return res
            .status(404)
            .json({ type: "danger", message: "no_country" });
    ...
    const countries = await new Countries({ iso: iso })
      ...
      .then(async (response) => {
        ...
        if (response != null)
          return res
            .status(404)
            .json({ type: "danger", message: "existed_country" });
        if (response === null) {
          await new Countries({ name, iso })
            .save()
            .then((result) => {
...
const getSingleCity = async (req, res) => {
  ...
  try {
    const cities = await new Cities({ id: city_id })
      ...
      .then(async (result) => {
        if (result === null)
          return res.status(404).json({ type: "danger", message: "no_city" });
      ...
      .then(async (response) => {
        if (response === null)
          return res
            ...
            .json({ type: "danger", message: "no_country" });
        if (response) {
          await new Cities({ country, iso, city, lat, lon })
            .save()
            .then((result) => {
              return res.status(201).json({ type: "success", Cities: result });
            })
            .catch((err) => {
              return res
                .status(500)
                .json({ type: "danger", message: "error_save_City" });
            });
        }
      });
  } catch (error) {
        ...
        if (result) {
          ...
          result
            ...
            .catch((err) => {
              return res
                .status(500)
                .json({ type: "danger", message: "error_edit_city" });
            });
        }
      });
  } catch (error) {
    ...
    const cities = new Cities({ id: city_id })
      ...
      .then(async (result) => {
        ...
        if (result) {
          ...
          return res
            .status(201)
            .json({ type: "success", message: "city_deleted" });
        }
      })
      .catch((err) => {
        return res
          .status(500)
          .json({ type: "danger", message: "error_delete_city" });
      });
  } catch (error) {
    res.status(500).json({ type: "danger", message: error });
  }
...
const getSingleCrop = async (req, res) => {
  ...
  try {
    const crops = await new Croptype({ id: crop_id })
      ...
      .then(async (result) => {
        if (result === null)
          return res.status(404).json({ type: "danger", message: "no_crop" });
...
const uploadVarietyPicture = async (req, res) => {
  uploadVariety(req, res, function (err) {
    if (err) {
      ...
      const varieties = new CropVarieties({
    ...
    if (crop && plant_date) {
      await new Croptype({
        ...
        .catch((err) => {
          return res
            .status(500)
            .json({ type: "danger", message: "error_save_Crops" });
        });
    }
  } catch (error) {
...
const getCrops = async (req, res) => {
  try {
    const crops = await new Croptype()
      ...
      .then(async (result) => {
        if (result == null)
          return res.status(404).json({ type: "danger", message: "no_crops" });
        if (result)
          return res.status(201).json({ type: "success", Crops: result });
      });
  } catch (error) {
    ...
    const cities = new Croptype({ id: crop_id })
      ...
      .then(async (result) => {
        ...
        if (result) {
          ...
          return res
            .status(201)
            .json({ type: "success", message: "crop_deleted" });
        }
      })
      .catch((err) => {
        return res
          .status(500)
          .json({ type: "danger", message: "error_delete_rop" });
      });
  } catch (error) {
    res.status(500).json({ type: "danger", message: error });
  }
...
const addVariety = async (req, res) => {
  ...
  if (!req.body.crop_id || req.body.crop_id == "")
    return res
      .status(404)
      .json({ type: "danger", message: "no_crop_selected" });
  try {
    const crops = new Croptype({ id: crop_id, deleted_at: null })
      .fetch({ require: false })
      .then(async (result) => {
        ...
        if (result) {
          await new CropVarieties({
            ...
            .then((result) => {
              return res
                .status(201)
                .json({ type: "success", cropVariety: result });
            })
            .catch((err) => {
              return res.status(500).json({ type: "danger", message: err });
            });
    ...
    const cropsVariety = await new CropVarieties({ id: variety_id })
      ...
      .then(async (result) => {
        if (result === null)
          return res
            .status(404)
            .json({ type: "danger", message: "no_variety" });
    ...
    const crops = await new CropVarieties()
      ...
      .then(async (result) => {
        if (result == null)
          return res
            .status(404)
            .json({ type: "danger", message: "no_varieties" });
        if (result)
          return res.status(201).json({ type: "success", Varieties: result });
      });
  } catch (error) {
    return res.status(500).json({ type: "danger", message: "error_user" });
  }
        ...
        if (result) {
          ...
          result
            ...
            .then((result) => {
              ...
              return res
                .status(201)
                .json({ type: "success", CropVariety: result,recalculation_message: recalculation});
            })
            .catch((err) => {
              return res.status(500).json({ type: "danger", message: err });
            });
    ...
    const variety = new CropVarieties({ id: variety_id })
      ...
      .then(async (result) => {
        ...
        if (result) {
          ...
          return res
            .status(201)
            .json({ type: "success", message: "crop_variety_deleted" });
        }
      })
      .catch((err) => {
        return res
          .status(500)
          .json({ type: "danger", message: "error_delete_variety" });
      });
  } catch (error) {
    res.status(500).json({ type: "danger", message: error });
  }
...
const getSingleSoil = async (req, res) => {
  ...
  try {
    const soils = await new Soiltype({ id: soil_id })
      ...
      .then(async (result) => {
        if (result === null)
          return res.status(404).json({ type: "danger", message: "no_soil" });
...
const addSoils = async (req, res) => {
  ...
  try {
    await new Soiltype({ soil, ru, pwp, taw, fc, practical_fraction, rain_eff })
      .save()
      .then((result) => {
      ...
      .catch((err) => {
        return res
          .status(500)
          .json({ type: "danger", message: "error_save_Soils" });
      });
  } catch (error) {
    res.status(500).json({ type: "danger", message: "error_user" });
  }
...
const getSoils = async (req, res) => {
  try {
    const soils = await new Soiltype()
      ...
      .then(async (result) => {
        if (result == null)
          return res.status(404).json({ type: "danger", message: "no_soils" });
        if (result)
          return res.status(201).json({ type: "success", Soils: result });
      });
  } catch (error) {
...
const editSoil = async (req, res) => {
  const { soil_id, soil, ru, pwp, taw, fc, practical_fraction, rain_eff } =
    req.body;
    ...
    const soils = new Soiltype({ id: soil_id })
      ...
      .then(async (result) => {
        ...
        if (result) {
          ...
          return res
            .status(201)
            .json({ type: "success", message: "soil_deleted" });
        }
      })
      .catch((err) => {
        return res
          .status(500)
          .json({ type: "danger", message: "error_delete_soil" });
      });
  } catch (error) {
    res.status(500).json({ type: "danger", message: error });
  }
    ...
    const irrigations = await new Irrigationtype({ id: irrigation_id })
      ...
      .then(async (result) => {
        if (result === null)
          return res
            .status(404)
            .json({ type: "danger", message: "no_irrigation" });
...
const addIrrigations = async (req, res) => {
  ...
  try {
    await new Irrigationtype({ irrigation, pivot_shape, lateral, effIrrig })
      .save()
      .then((result) => {
      ...
      .catch((err) => {
        return res
          .status(500)
          .json({ type: "danger", message: "error_save_Irrigations" });
      });
  } catch (error) {
    res.status(500).json({ type: "danger", message: "error_user" });
  }
...
const editIrrigation = async (req, res) => {
  const { irrigation_id, irrigation, pivot_shape, lateral, effIrrig } =
    req.body;

  try {
    const irrigations = new Irrigationtype({
      id: irrigation_id,
      deleted_at: null,
    })
      .fetch({ require: false })
        ...
        if (result) {
          ...
          result
            ...
            .then((result) => {
              return res
                .status(201)
                .json({ type: "success", Irrigations: result });
            })
            .catch((err) => {
              return res.status(500).json({ type: "danger", message: err });
            });
    ...
    const irrigations = new Irrigationtype({ id: irrigation_id })
      ...
      .then(async (result) => {
        ...
        if (result) {
          ...
          return res
            .status(201)
            .json({ type: "success", message: "irrigation_deleted" });
        }
      })
      .catch((err) => {
        return res
          .status(500)
          .json({ type: "danger", message: "error_delete_irrigation" });
      });
  } catch (error) {
    res.status(500).json({ type: "danger", message: error });
  }
