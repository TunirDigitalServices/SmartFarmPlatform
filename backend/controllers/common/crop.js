    ...
    const user = await new User({ uid: uid })
      .fetch({
        withRelated: [
          ...
          {
            "farms.fields.crops": (qb) => {
              qb.where("deleted_at", null);
            },
          },
          {
            "farms.fields.crops.irrigations": (qb) => {
              qb.where("deleted_at", null);
            },
          },
          {
            "farms.fields.crops.croptypes": (qb) => {
              qb.where("deleted_at", null);
            },
          },
        ],
    ...
    const field = new Field({ uid: fieldUid, deleted_at: null })
      ...
      .then(async (result) => {
        if (result === null)
          return res
            .status(404)
            .json({ type: "danger", message: "no_field_crop" });
        if (result) {
          return res
            .status(201)
            .json({ crops: result.related("crops"), field: result });
        }
      })
      .catch((err) => {
        return res
          .status(500)
          .json({ type: "danger", message: "error_get_field" });
      });
  } catch (error) {
    return res.status(500).json({ type: "danger", message: "error_user" });
  }
...
const addCrop = async (req, res) => {
  if (!req.body.field_uid || req.body.field_uid == "")
    return res
      .status(404)
      .json({ type: "danger", message: "no_field_selected" });
  ...
  const zone = await new Zone({ uid: zone_uid, deleted_at: null })
    ...
    .then(async (result) => {
      if (result === null)
        return res
          .status(404)
          .json({ type: "danger", message: "no_zone_selected" });
        ...
        if (result) {
          ...
          result
            ...
            .catch((err) => {
              return res
                .status(500)
                .json({ type: "danger", message: "error_edit_crop" });
            });
        }
      });
  } catch (error) {
  ...
  try {
    const crop = new Crop({ uid: crop_uid })
      ...
      .catch((err) => {
        return res
          .status(500)
          .json({ type: "danger", message: "error_delete_crop" });
      });
  } catch (error) {
...
const validateCrop = (method) => {
  switch (method) {
    case "addEdit": {
      return [body("croptype_id", "type_empty").notEmpty()];
    }
  }
};
module.exports = {
  getCropsByConnectedUser,
  getSingleCrop,
  getCropsByField,
  validateCrop,
  addCrop,
