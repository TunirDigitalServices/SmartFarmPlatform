...
const addZone = async (req,res) => {
    const {name,description,source,depth_data,field_uid,RUmax,effPluie,irrigArea} = req.body;
    //exemple data
    /*{"name":'zone1',
    ...
    try {
        ...
        .then(async result=>{
            ...
            if (result) {
                await new Zone({name,description,source,RUmax,effPluie,irrigArea,depth_data : depthData,field_id : result.get('id')}).save()
                .then(result=>{
                    return res.status(201).json({ type:"success", zone : result })
                })
                .catch(err=>{
