...
function isCloud(scl) {
  ...
  return false;
    ...
    if (!tokenJson.access_token) {
      return res.status(401).json({ type: "danger", message: "Failed to get Sentinel token", details: tokenJson });
    }
      ...
      for (const farm of user.farms || []) {
        for (const field of farm.fields || []) {
          ...
          for (const type of Object.keys(evalscripts)) {
            try {
              const imageRes = await fetch("https://services.sentinel-hub.com/api/v1/process", {
                method: "POST",
                headers: {
              ...
              if (!imageRes.ok) {
                console.error(`Failed image fetch for ${type} (User ${user.id}, Field ${field.id})`, await imageRes.text());
                continue;
              }
              ...
              const staticUrl = await saveToStaticServer(user.id, field.id, buffer, fileName);

              await new SatelliteImage({
                user_id: user.id,
                field_id: field.id,
              ...
              console.error(`Error processing ${type} for field ${field.id}:`, err.message);
            }
          }
        }
      }
...
function getBboxFromPolygon(polygon) {
  ...
  return [Math.min(...longs), Math.min(...lats), Math.max(...longs), Math.max(...lats)];
}
...
async function saveToStaticServer(userId, field_id, buffer, fileName) {
  ...
  return `/static/satellite-images/user_${userId}/field_${field_id}/${today}/${fileName}`;
}
