...
const generatePDF = async (req, res) => {
    ...
    new CalculSensor()
        .query((qb) => qb.where('deleted_at', null).andWhere('start_date', '<=', today).andWhere('end_date', '>=', today))
        .fetchAll({
            withRelated: [
                { 'sensors': (qb) => { qb.where('deleted_at', null).andWhere('synchronized', "1").and.whereNotNull('Latitude').and.whereNotNull('Longitude') } },
                { 'users': (qb) => { qb.where('deleted_at', null).andWhere('is_active', "1").andWhere('is_valid', "1"); }, 'users.farms': (qb) => { qb.where('deleted_at', null); } },
                { 'fields': (qb) => { qb.where('deleted_at', null); }, 'fields.crops': (qb) => { qb.where('deleted_at', null); }, 'fields.crops.croptypes': (qb) => { qb.where('deleted_at', null); }, 'fields.crops.varieties': (qb) => { qb.where('deleted_at', null); }, 'fields.crops.irrigations': (qb) => { qb.where('deleted_at', null); }, 'fields.zones': (qb) => { qb.where('deleted_at', null); }, 'fields.zones.soiltypes': (qb) => { qb.where('deleted_at', null); } }],
            require: false
        })
        .then(async result => {
            let data = JSON.parse(JSON.stringify(result));
            await Promise.all(data.map(async dataCalcul => {
                let calcul = []
                let arrWeather = []
                let cropType = ""
                let plantDate = ""
                ...
                allDataSensor.forEach(async sensorData => {
                    ...
                    if (!dateTemperatureMap.has(date)) {
                        ...
                        let tempMax = Math.max(...tempArr);
                        dateTemperatureMap.set(date, temperature);
                        dateTemperatureMinMap.set(date, Math.min(tempMin, dateTemperatureMinMap.get(date)));
                        dateTemperatureMaxMap.set(date, Math.max(tempMax, dateTemperatureMaxMap.get(date)));
                    }
                    ...
                    if (!dateHumidityMap.has(date)) {
                        ...
                        let humidityMax = Math.max(...humdArr);
                        dateHumidityMap.set(date, humidity);
                        dateHumidityMinMap.set(date, Math.min(humidityMin, dateHumidityMinMap.get(date)));
                        dateHumidityMaxMap.set(date, Math.max(humidityMax, dateHumidityMaxMap.get(date)));
                    }
                    ...
                    if (!datePressureMap.has(date)) {
                        ...
                        datePressureMinMap.set(date, Math.min(pressureMin, datePressureMinMap.get(date)));
                        datePressureMaxMap.set(date, Math.max(pressureMax, datePressureMaxMap.get(date)));
                    }
                    let humidityNv1 = parseFloat(((4095 - Number(sensorData.mv1)) / (4095 - 600)) * 100).toFixed(2)
                    niveau1.push(sensorData.mv1)
                    // let Mv1Min = Math.min(...niveau1);
                    // let Mv1Max = Math.max(...niveau1);
                    let humidityNv2 = parseFloat(((4095 - Number(sensorData.mv2)) / (4095 - 600)) * 100).toFixed(2)
                    niveau2.push(sensorData.mv2)
                    // let Mv2Min = Math.min(...niveau2);
                    // let Mv2Max = Math.max(...niveau2);
                    let humidityNv3 = parseFloat(((4095 - Number(sensorData.mv3)) / (4095 - 600)) * 100).toFixed(2)
                ...
                if (dataCalcul) {
                    if (dataCalcul.fields.crops) {
                        dataCalcul.fields.crops.map(crops => {
                            ...
                            if (typeof crops.varieties.crop_variety !== 'undefined' && crops.varieties.crop_variety != null && crops.varieties.crop_variety != "") {
                ...
                const html = fs.readFile(path.join(__dirname, '../../reports/template.html'), 'utf-8', async function (err, data) {
                    ...
                    await pdf.create(
                        {
                            ...
                            data: {
                                products: [{
                                    ...
                                    'Lat': parseFloat(dataCalcul.sensors.Latitude).toFixed(4),
                                    'Lon': parseFloat(dataCalcul.sensors.Longitude).toFixed(4),
                                    'crop': cropType,
                                    'irrigtype': irrigType,
                                    'flowrate': flowrate,
                                    ...
                                    'plantDate': new Date(plantDate).toLocaleString().slice(0, 10),
                                    'soil': soilType,
                                    'startDate': addDays(startDate, -7).toLocaleString().slice(0, 10),
                                    'endDate': addDays(endDate, -7).toLocaleString().slice(0, 10),
                                    'startDateWeather': new Date(startDate).toLocaleString().slice(0, 10),
                                    'endDateWeather': new Date(endDate).toLocaleString().slice(0, 10),
...
const createBulletin = async (req, res) => {
    ...
    try {
        ...
        let today = todayDate.toISOString().slice(0, 10)
        new CalculSensor()
            .query((qb) => qb.where('deleted_at', null).andWhere('start_date', '<=', addDays(today, +1).toISOString().slice(0, 10)).andWhere('end_date', '>=', addDays(today, +1).toISOString().slice(0, 10)))
            .fetchAll({
                withRelated: [
                    { 'sensors': (qb) => { qb.where('deleted_at', null).andWhere('synchronized', "1").and.whereNotNull('Latitude').and.whereNotNull('Longitude') } },
                    { 'users': (qb) => { qb.where('deleted_at', null).andWhere('is_active', "1").andWhere('is_valid', "1"); }, 'users.farms': (qb) => { qb.where('deleted_at', null); } },
                    { 'fields': (qb) => { qb.where('deleted_at', null); }, 'fields.crops': (qb) => { qb.where('deleted_at', null); }, 'fields.crops.croptypes': (qb) => { qb.where('deleted_at', null); }, 'fields.crops.varieties': (qb) => { qb.where('deleted_at', null); }, 'fields.crops.irrigations': (qb) => { qb.where('deleted_at', null); }, 'fields.zones': (qb) => { qb.where('deleted_at', null); }, 'fields.crops.irrigations': (qb) => { qb.where('deleted_at', null); }, 'fields.zones': (qb) => { qb.where('deleted_at', null); }, 'fields.zones.soiltypes': (qb) => { qb.where('deleted_at', null); } }],
                require: false
            })
            .then(async result => {
                let data = JSON.parse(JSON.stringify(result));
                        ...
                        await pdf.create(
                            {
                                ...
                                data: {
                                    products: [{
                                        ...
                                        'soil': soilType,
                                        'startDate': new Date(startDate).toLocaleString().slice(0, 10),
                                        'endDate': addDays(endDate, -1).toLocaleString().slice(0, 10),
                                        "totalIrrigation": parseFloat(total).toFixed(1),
                                        "irrigNumber": irrigNumber,
                    ...
                    await Promise.all(data.map(async (fields) => {
                        ...
                        if (fields.crops !== []) {
                            ...
                            cropsData.map(async crops => {
                                ...
                                if (typeof crops.dose_efficiency !== 'undefined' && crops.dose_efficiency !== null) {
...
const sendSMStoUsers = async (phoneNumber, irrigNumber, irrigDates, irrigTime) => {
    ...
    try {
        const response = await fetch(`https://www.winsmspro.com/sms/sms/api?action=send-sms&api_key=${apiKey}&to=216${phoneNumber}&from=${from}&sms=${message}`)
        if (!response.ok) {
...
const sendSMStoSelectedUser = async (req, res) => {
    ...
    try {
        ...
        if (jsonData.code === "ok") {
            console.log("SMS sent successfully!");
            return res.status(200).json({ type: "success", message: "SMS sent successfully" });
        } else {
            console.log("Failed to send SMS.");
            return res.status(500).json({ type: "danger", message: "Error when sending SMS" });
        ...
        await new CalculSensor()
            ...
            .then(async result => {
                ...
                if (filteredResult.length > 0) {
                    ...
                    description = `Le bulletin hebdomadaire du ${toSqlDate(addDays(startDate, +1))} au ${toSqlDate(addDays(endDate, +1))} est prêt , n'hésitez pas à nous contacter si vous avez des questions ou des commentaires sur ce bulletin. `
        ...
        await new CalculSensor()
            ...
            .then(async result => {
                ...
                if (filteredResult.length > 0) {
                    ...
                    const hours = Math.floor(irrigTime);
                    let irrigationDuration = `${hours}h`;
                    let message = `Il est recommandé de répartir les besoins en eau de votre parcelle sur ${irrigNbr} fois par semaine. Pour la prochaine irrigation, prévoyez les dates suivantes : ${irrigDates.join(", ")} pendant ${irrigationDuration} minutes.`
                    ...
                    await new Recommendation({ title, description, message, user_id, field_id }).save()
                }
            })
    } catch (error) {
        console.log(error)
...
const getReportsByField = async (req, res) => {
    ...
    try {
        const field = new Field({ 'id': fieldId, deleted_at: null })
            .fetch({ withRelated: [{ 'reports': (qb) => { qb.where('deleted_at', null).orderBy('id', 'DESC') } }] })
            .then(async result => {
