...
const createReport = async (req, res) => {
    try {
        ...
        const calculResults = await new CalculSensor()
            ...
            .fetchAll({
                withRelated: [
                    ...
                    { 
                        'fields': qb => { qb.where('deleted_at', null); },
                        'fields.crops': qb => { qb.where('deleted_at', null); },
                        'fields.crops.croptypes': qb => { qb.where('deleted_at', null); },
                        'fields.crops.varieties': qb => { qb.where('deleted_at', null); },
                        'fields.crops.irrigations': qb => { qb.where('deleted_at', null); },
                        'fields.zones': qb => { qb.where('deleted_at', null); },
                        'fields.zones.soiltypes': qb => { qb.where('deleted_at', null); }
                    }
                ],
                require: false
            });
        ...
        return res.status(200).json({ type: "success", message : 'Reports Created Successfully' });
...
const processCalculation = async (dataCalcul) => {
    try {
        ...
        // Generate PDF report
        await generatePDFReport(dataCalcul, fieldInfo, calcul, bilan, irrigationStats, irrigationDuration);
        ...
        for (const crop of dataCalcul.fields.crops) {
            ...
            if (crop.irrigations && crop.irrigations.length > 0) {
                for (const irrig of crop.irrigations) {
                    info.irrigType = irrig.type === "drip" ? "Goutte à goutte" : irrig.type;
...
const processCalculResults = (dataCalcul) => {
    ...
    const filteredResult = (dataCalcul.result || []).filter(result => {
        ...
        return toSqlDate(startDate) <= resultDate && 
    ...
    if (filteredResult.length > 0) {
        for (let i = 0; i < filteredResult.length - 1; i++) {
            ...
            if (element.bilan && element.bilan.length > 0) {
                const averageBilanValue = element.bilan.reduce(
    ...
    const calculIrrigation = calcul.filter(irrig => Number(irrig.irrigation) !== 0);
    irrigNumber = calculIrrigation.length;
    
    if (calculIrrigation.length > 0) {
        // Get irrigation time from last irrigation record
        irrigTime = parseFloat(calculIrrigation[calculIrrigation.length - 1].irrigationTime || 0).toFixed(0);
...
// Generate PDF report
const generatePDFReport = async (dataCalcul, fieldInfo, calcul, bilan, irrigationStats, irrigationDuration) => {
    const startDate = new Date(dataCalcul.start_date);
    const endDate = new Date(dataCalcul.end_date);
    const newStartDate = startDate.getDate().toString().padStart(2, '0') + '/' + 
                 (startDate.getMonth() + 1).toString().padStart(2, '0') + '/' + 
                 startDate.getFullYear();
    const newEndDate = endDate.getDate().toString().padStart(2, '0') + '/' + 
    ...
    try {
        ...
        if (!isScheduledExecutionTime()) {
            return res.status(200).json({ 
                type: 'success', 
                message: 'Calculation skipped. Calculations should start on Mondays between 2:00 AM and 3:00 AM.' 
            });
        }
...
const processField = async (field) => {
    try {
        ...
        if (field.sensors && field.sensors.length > 0) {
            ...
            console.warn(`No sensors for field ${field.id}, proceeding without sensors`);
...
const processSensors = async (
    ...
    // Process each sensor
    await Promise.all(field.sensors.map(async (sensor) => {
        await processSensor(
            sensor,
            field,
            dataCrop,
...
const processSensor = async (
    ...
    try {
        ...
        console.log(`Processing sensor ${sensor ? sensor.id : 'null'} for field ${field.id} with user_id: ${userId}`);

        if (!codeSensor && (!dataCrop || !Object.keys(dataCrop).length || !dataCrop.all_kc || days <= 0 || !field.Latitude || !field.Longitude)) {
            console.warn(`Skipping calculation for field ${field.id} due to missing data`);
            return;
        }
        ...
        console.log(`Saving calculation for field ${field.id}, sensor ${codeSensor}, user ${userId}`);
        ...
        console.error(`Error processing sensor ${sensor ? sensor.id : 'null'} for field ${field.id}:`, error);
    }
};
...
const saveCalculationResults = async (
    ...
    try {
        ...
        if (!userId) {
            console.warn(`⚠️ Warning: user_id is NULL when saving calculation for field ${fieldId}`);
        }
...
module.exports = {
    calculSensorBilan , createReport
};